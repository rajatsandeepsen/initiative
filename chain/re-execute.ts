...
export type Execution<U extends State, A extends AvailableActions, P> = Awaited<
	ReturnType<typeof executeChainActions<U, A, {}>>
>;
...
>(
	...
	const executions = await ArrayMapAsync(
		...
		async (execute, i, [err, setError]) => {
			...
			const { key, value } = execute as unknown as {
				key: string;
				value: Parameters<Infer<A[keyof A]>>[0];
			};
